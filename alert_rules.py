...
RULE_TYPES = {
    ...
    "Opportunity At Least": "opportunity_min", "AI Confidence At Least": "confidence_min",
...
def evaluate_rule(rule: Dict[str, Any], snapshot: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    mapping = {
        ...
        "opportunity_min": ai.get("opportunity_score"), "confidence_min": ai.get("ai_confidence"),
    ...
    if kind in mapping:
        ...
        met = current >= threshold if kind != "price_below" else current <= threshold
        return met, f"Current {current:g}; threshold {threshold:g}."
    text_map = {
        "grade_equals": tech.get("grade"), "setup_contains": tech.get("setup"),
