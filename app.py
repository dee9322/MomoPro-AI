        ...
        if matching_rows.empty:
            ...
            if comparison_symbol != "None":
                ...
                if not comparison_rows.empty:
                    comparison_payload = comparison_rows.iloc[0].to_dict()
            ...
            top_cols[0].metric("Ticker", analysis_symbol)
            top_cols[1].metric("Grade", analysis_stock.get("Grade", "—"))
            top_cols[2].metric(
                "Technical Confidence",
                percent_text(analysis_stock.get("Momo Confidence")),
            )
            top_cols[3].metric("Momo Engine", momo_view.get("decision", "—"))
            top_cols[4].metric(
                "Relative Strength",
                analysis_rs.get("verdict", "—") if isinstance(analysis_rs, dict) else "—",
            )

            report_key = (
                f"{analysis_symbol}|{comparison_symbol}"
                if comparison_symbol != "None"
            ...
            if st.button(
                ...
                try:
                    with st.spinner("Researching the full MomoPro evidence package..."):
                        news_package = load_ticker_news(analysis_symbol)
                        ranked_news = rank_news(news_package.get("articles", []))
                        news_summary = summarize_news(ranked_news)

                        try:
                            sec_filings = load_sec_filings(analysis_symbol)
                        except Exception:
                            sec_filings = []
                        ...
                        try:
                            fda_records = load_fda_records(company_name)
                        except Exception:
                            fda_records = []
            ...
            if report:
                ...
                summary_cols[2].metric("Conviction", report.get("conviction", "—"))
                summary_cols[3].metric("Risk", report.get("risk_level", "—"))
                summary_cols[4].metric("Final Rating", report.get("final_rating", "—"))
