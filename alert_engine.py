...
def create_rule(name, symbols, rule_type, value, cooldown_hours=24):
    data = load_alerts()
    rule = {"id": uuid.uuid4().hex, "name": name.strip() or rule_type, "symbols": sorted(set(symbols)), "type": rule_type, "value": value, "enabled": True, "cooldown_hours": int(cooldown_hours), "last_triggered": {}, "created_at": _iso()}
...
def evaluate_alerts(items_by_symbol: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for rule in data["rules"]:
        ...
        for symbol in rule.get("symbols", []):
            ...
            if last:
                try:
                    if _now() - datetime.fromisoformat(last) < timedelta(hours=int(rule.get("cooldown_hours", 24))):
                        continue
                except ValueError:
                    pass
            ...
            if met:
                ...
                rule.setdefault("last_triggered", {})[symbol] = event["timestamp"]
